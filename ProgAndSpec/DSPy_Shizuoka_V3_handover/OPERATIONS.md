...
prob_082（キャッシュフローマッチング）は問題文にも instance にも債券の額面がなく、参照解を
一意に定められないため雛形にしていません。金融・投資_線形計画 は prob_083（MAD
ポートフォリオ）で代表させています。
...
- Qwen3.8 は思考が `max_tokens` 32,768 を使い切って本文が空になる問題が 140 問中 21〜35 問
  あります（`RESCORE_REPORT.md` 14 章）。モデル比較では思考予算を条件として明示してください。
