...
def seed_best_known_multiroll(
    ...
    for i, ex in enumerate(examples):
        ...
        status = (
            "improved"
            if best_cost is not None and ref is not None and best_cost < ref
            else "baseline"
        )
...
def train_iterative(
    ...
    print(
        f"  Kept {len(bootstrapped)}/{min(max_bootstrapped, len(train_examples))} valid bootstraps"
    )
        ...
        for i, ex in enumerate(train_examples):
            try:
                ...
                if score < 1.0:
                    ...
                    try:
                        parse_code = generator.parse_code_dict.get(
                            ex.core_type, default_parse_code(ex.instance)
                        )
                        ...
                        if improved_score > score:
                            ...
                            print(
                                f"  [{i + 1}] {ex.instance_id}: {score:.2f} -> {improved_score:.2f} (improved) [{err_cat}->{improved_err}]"
                            )
                        else:
                            print(
                                f"  [{i + 1}] {ex.instance_id}: {score:.2f} -> {improved_score:.2f} (no improvement) [{err_cat}->{improved_err}]"
                            )
                            del improved_codes[ex.instance_id]  # Discard worse code
                    except Exception as e:
                        print(
                            f"  [{i + 1}] {ex.instance_id}: score={score:.2f} [{err_cat}] (improve failed: {e})"
                        )
                else:
...
def evaluate(generator: AlgorithmGenerator, examples: list[dict], name: str = "Test") -> dict:
    ...
    for i, ex in enumerate(examples):
        ...
        try:
            ...
            results.append(
                {"instance_id": iid, "status": "gen_error", "score": 0.0, "error": str(e)}
            )
            continue
        ...
        print(
            f"  [{i + 1}/{len(examples)}] {iid}: score={score:.2f} status={status} cost={cost_str} ref={ref_str}"
        )
...
def save_program(generator: AlgorithmGenerator, path: str) -> None:
    ...
    for predictor in generator.predictors():
        ...
        if sig:
            program_data[f"cot_{len(program_data) - 1}"]["signature_name"] = (
                sig.__name__ if hasattr(sig, "__name__") else str(sig)
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Legacy V3 iterative Train & Eval")
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--data-dir", default=str(BASE_DIR / "data" / "problems"), help="データディレクトリ"
    )
    parser.add_argument("--train-ratio", type=float, default=0.8, help="訓練データの割合")
    parser.add_argument("--iterations", type=int, default=3, help="訓練の反復回数")
    parser.add_argument(
        "--program-path",
        default=str(BASE_DIR / "compiled_program_v3_gepa_phaseE.json"),
        help="プログラムの保存/読込パス",
    )
    parser.add_argument("--bootstrap", type=int, default=3, help="ブートストラップ例数")
    parser.add_argument("--seed-rolls", type=int, default=2, help="best_known播种のロール数")
    ...
    if args.mode in ("train", "full"):
        ...
        generator = train_iterative(
            train_only, val_examples, n_iterations=args.iterations, max_bootstrapped=args.bootstrap
        )
