...
def summarize_reference_solution(ref_sol: dict, instance: dict) -> str:
    ...
    # Objective value
    for obj_key in ["objective_value", "makespan", "total_distance", "total_cost", 
                    "project_duration", "num_trains", "total_delay"]:
        if obj_key in ref_sol and isinstance(ref_sol[obj_key], (int, float)):
            lines.append(f"- Objective ({obj_key}) = {ref_sol[obj_key]} (target to match or beat)")
            break
    ...
    if "schedule" in ref_sol:
        ...
        if isinstance(sched, dict):
            lines.append(f"- 'schedule' is a dict with {len(sched)} entries (likely job_id → operations list)")
            sample_key = next(iter(sched), None)
            if sample_key is not None:
                sample = sched[sample_key]
                if isinstance(sample, list) and sample:
                    if isinstance(sample[0], dict):
                        lines.append(f"  Each schedule entry is a list of operations. Example op keys: {list(sample[0].keys())}")
        elif isinstance(sched, list):
            lines.append(f"- 'schedule' is a list of {len(sched)} entries")
    ...
    if "routes" in ref_sol:
        ...
        if isinstance(routes, list):
            ...
            lines.append(f"- 'routes' is a list of {num_routes} routes")
            if routes and isinstance(routes[0], dict):
                ...
                lines.append(f"  Each route is a list of node IDs. Sample: {routes[0][:8]}")
            # Reference vehicle usage
            customers = instance.get("customers", [])
            if customers:
                total_nodes = sum(len(r) if isinstance(r, list) else len(r.get("route", r.get("path", []))) if isinstance(r, dict) else 0 for r in routes)
                lines.append(f"  Reference uses {num_routes} routes to visit {len(customers)} customers (avg {total_nodes/max(num_routes,1):.1f} stops per route)")
        elif isinstance(routes, dict):
            lines.append(f"- 'routes' is a dict with {len(routes)} entries")
    ...
    if "assignments" in ref_sol or "assignment" in ref_sol:
        ...
        if isinstance(assigns, list):
            lines.append(f"- 'assignments' is a list of {len(assigns)} entries")
        elif isinstance(assigns, dict):
            lines.append(f"- 'assignments' is a dict with {len(assigns)} entries")
    ...
    if "sequence" in ref_sol or "optimal_sequence" in ref_sol:
        ...
        if isinstance(seq, list):
            lines.append(f"- 'sequence' is a list of {len(seq)} elements (job order). Sample: {seq[:5]}")
...
def build_requirement(record: dict) -> str:
    """
    ...
    if ref is not None:
        ...
        parts.append("Your algorithm should try to achieve a value close to or better than this reference (lower is better).")

    # Reference solution structure hint (NEW)
    ref_sol = record.get("reference_solution", {})
    if ref_sol:
        ref_summary = summarize_reference_solution(ref_sol, instance)
        if ref_summary:
            parts.append("## Reference Solution Structure")
            parts.append(ref_summary)
            parts.append("Your solve() function should return a solution with the SAME top-level structure. "
                        "Match the field names (e.g., 'schedule', 'routes', 'makespan') exactly.")
    ...
    parts.append("Your solve() function MUST return the solution as a Python value (list, dict, int, etc.).")
    ...
    parts.append("CRITICAL: An empty solution (empty schedule, empty routes, cost=0) will be REJECTED. "
